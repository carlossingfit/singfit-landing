...
const PAGE_ID = "caregiverfreetrial";
...
function buildCheckoutUrl() {
  ...
  ATTRIBUTION_PARAMS.forEach((param) => {
    ...
    if (currentValue) {
      sessionStorage.setItem(`caregiverfreetrial_${param}`, currentValue);
      checkoutUrl.searchParams.set(param, currentValue);
      return;
    }

    const storedValue = sessionStorage.getItem(`caregiverfreetrial_${param}`);

    if (storedValue) {
      checkoutUrl.searchParams.set(param, storedValue);
    }
  });
...
function trackCta(buttonText, destinationUrl, ctaLocation) {
  pushDataLayer("click_cta", {
    ...
    offer_type: "30_day_free_trial",
    price_after_trial: 11.99,
  });
}
...
export default function CaregiverFreeTrial() {
...
>
            ...
            <aside id="mobile-menu-drawer" className="mobile-drawer" aria-label="Mobile navigation" aria-hidden={!menuOpen}>
              ...
              <nav className="drawer-nav">
                {[
                  ...
                  ["Stories", "#stories"],
                  ["Questions", "#questions"],
                  ["Start 30-Day Free Trial", checkoutUrl,
],
                ].map(([label, href]) => (
  ...
  <strong>30 days free, then $11.99/month. Cancel anytime.</strong>
  <span>Credit card required. Trial automatically renews unless canceled before the trial ends.</span>
</div>
            </aside>
          </header>
...
/>
  ...
  <div className="price-card">
    <div className="price-top">
      ...
      <p className="price-sub">for your first 30 days</p>
    </div>
